...
fig1.add_trace(go.Scatter(x=x, y=pdf, mode='lines', line=dict(color='black', width=2), showlegend=False))
...
window = st.slider("Rolling window size", 2, len(returns), min(20, len(returns)))
ann_factor = np.sqrt({"Daily":252, "Weekly":52, "Monthly":12}[periodicity])
data['RollingVol'] = data['Return'].rolling(window).std() * ann_factor
...
fig2.add_trace(go.Scatter(x=rv.index, y=rv['Close'], mode='lines', name='Close Price',line=dict(color='black', width=1.5)), secondary_y=True)
...
if use_em and exp_move is not None:
    lower_thr = -exp_move; upper_thr = exp_move
else:
...
fig3.add_trace(go.Histogram(x=returns, histnorm='probability density', nbinsx=50,
    marker=dict(color='lightgrey', line=dict(color='black', width=1)), showlegend=False))
fig3.add_trace(go.Scatter(x=x, y=pdf, mode='lines', line=dict(color='black', width=2), showlegend=False))
