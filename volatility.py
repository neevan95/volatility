...
if expiries:
    ...
    # --- Expiry-wide OI sums and Put/Call OI ratio ---
    ...
    # Build strikes list and choose default = nearest to latest close
    strikes_arr = np.array(sorted(pd.unique(calls['strike'].dropna().astype(float))))
    ...
    if sel_strike is not None:
        # IV at selected strike
        call_iv_s = calls.loc[calls['strike'] == sel_strike, 'impliedVolatility']
        put_iv_s  = puts.loc[puts['strike']  == sel_strike, 'impliedVolatility']
        call_iv = float(call_iv_s.iloc[0]) if not call_iv_s.empty and not pd.isna(call_iv_s.iloc[0]) else np.nan
        put_iv  = float(put_iv_s.iloc[0])  if not put_iv_s.empty  and not pd.isna(put_iv_s.iloc[0])  else np.nan
        iv_strike = np.nanmean([call_iv, put_iv])

        # OI at selected strike (compact) + PCR(strike)
        def _safe_oi(df):
            if 'openInterest' in df.columns:
                s = df.loc[df['strike'] == sel_strike, 'openInterest']
                if not s.empty and not pd.isna(s.iloc[0]):
                    return int(s.iloc[0])
            return np.nan
        call_oi = _safe_oi(calls)
        put_oi  = _safe_oi(puts)
        pcr_strike_text = _pcr_text(put_oi, call_oi)

        st.write(f"Strike {sel_strike:,.2f}: Call IV {call_iv:.2%}, Put IV {put_iv:.2%}, Avg {iv_strike:.2%}")
        st.caption(
        ...
        exp_move_call = call_iv * np.sqrt(t_years) if t_years > 0 and not np.isnan(call_iv) else 0
        exp_move_put  = put_iv  * np.sqrt(t_years) if t_years > 0 and not np.isnan(put_iv)  else 0
